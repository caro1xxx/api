...
const Order = (props: Props) => {
  ...
  const { data, loading } = useRequest(() => getOrder(props.order), {
    cacheKey: "order",
    onSuccess: (result) => {
        ...
        data && data.code === 200 ? (
          <div className="body">
            ...
            <Space.Compact style={{ width: "100%" }}>
              ...
              <Button onClick={() => discount(data.order.no, state.code)}>使用折扣码</Button>
            </Space.Compact>
            ...
            <Button loading={state.loading} style={{ width: "100%" }} onClick={() => payment(data.order.no)}>
              付款
            </Button>
          </div>
        ) : (
